...
// Suite 10: 새 보석 수 공식
function calcColorsNew(level: number): number {
    return Math.min(12, Math.round(3 + (level - 1) * 9 / 99));
}

// Suite 11: 스크롤 관련 순수 함수
function calcMaxScrollY(screenHeight: number): number {
    const fixedCellH = 60;
    const startY = 80;
    return Math.max(0, (startY + 20 * fixedCellH) - screenHeight);
}

function clampScrollY(scrollY: number, screenHeight: number): number {
    return Math.max(0, Math.min(calcMaxScrollY(screenHeight), scrollY));
}

function getLevelFromClickWithScroll(
    clientX: number, clientY: number,
    screenWidth: number, scrollY: number
): number | null {
    const fixedCellH = 60;
    const startY = 80;
    const cols = 5;
...
describe("새 보석 수 공식 검증 (calcColorsNew)", () => {
    ...
    it("T10-8: 레벨 1~100 모두 3~12 범위", () => {
        for (let lv = 1; lv <= 100; lv++) {
            ...
            if (c < 3 || c > 12) throw new Error(`Level ${lv} returned ${c}`);
        }
    });
});
